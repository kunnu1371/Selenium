...
//	Post req body using external json file
	@Test
	void postAPITestWithExternalJSonFile() {

		File f = new File(
				"C:\\Users\\gautam.kunal\\eclipse-workspace\\DemoSelenium\\DemoSelenium\\src\\test\\resources\\data.json");

		given().body(f).contentType("application/json").when().post("https://reqres.in/api/users").then()
